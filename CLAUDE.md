...
- **M3 Visual System** (next code milestone) → **M4 Presets/Remix** → **M5 Fake Specs + Dual PNG Export** → **M6 Landing/QA/Deploy**: planned, plan files written just before each. M0 prerequisite is satisfied; write `docs/superpowers/plans/2026-06-02-visual-system.md` just before starting M3, sourcing theme tokens from `docs/reference/visual-direction.md` and the first hero chip from `docs/reference/hero-compositions.md` (composition A).
