...
- **v4 "Community"** is underway: **V4-M0 (moderation + access gate)** and **V4-M1 (reactions: likes + comments + report button)** are done on branch `v4-community` (not yet merged; all v4 milestones M0–M4 land on this branch). Next v4 milestones (M2 ranking, M3 contests, M4 remix lineage) are direction-approved but each gets its own brainstorm→spec→plan cycle. Roadmap: `docs/superpowers/specs/2026-06-12-v3-v4-roadmap-design.md`. Public launch is a separate gate; the access gate (`VSL_SIGNUPS_OPEN`, default `false`) is the mechanism for it.
