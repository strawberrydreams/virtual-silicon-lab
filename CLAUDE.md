...
- **M4 Presets/Remix** (next code milestone): detailed plan ready at `docs/superpowers/plans/2026-06-02-presets-and-remixing.md`. Start at Task 1. The plan materializes six immutable curated blueprints into fresh ordinary `Project` JSON, adds a project-store remix command, and replaces the M3 dashboard hero shortcut with lightweight preset preview cards.
- **M5 Fake Specs + Dual PNG Export** → **M6 Landing/QA/Deploy**: planned, detailed plan files written just before each milestone.
