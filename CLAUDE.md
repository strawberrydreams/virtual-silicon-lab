...
- **v7 "Visual Depth"** is in progress on branch `v7-visual-depth`; **V7-M0–M5 are complete (M4 intentionally dropped)**. The editor and public gallery share a lazy full-screen Three.js showcase derived from the same project snapshot, with PBR/PMREM/ACES/bloom, turntable/glow, editor-only deterministic MP4 export, and a 400-piece/WebGL admission budget that falls back to the poster. The server-rendered share viewer links to gallery 3D without client JS. Three, recorder, and `mp4-muxer` stay in lazy chunks; no schema/migration/API/upload change. M5 spec: `docs/superpowers/specs/2026-06-19-v7-m5-gallery-share-integration-design.md`.
...
### v7 Visual Depth (in progress — branch `v7-visual-depth`, not yet merged)
