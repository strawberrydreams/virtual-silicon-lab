...
- **M4 Presets/Remix** (next code milestone) → **M5 Fake Specs + Dual PNG Export** → **M6 Landing/QA/Deploy**: planned, plan files written just before each. Write `docs/superpowers/plans/2026-06-02-presets-and-remixing.md` just before starting M4; `createHeroChip` + theme tokens + `resolveBlockStyle` are the starting point for the preset catalog.
