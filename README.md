...
안전/모더레이션·온보딩·디스커버리·운영 하드닝**을 더했습니다.

> 버전 표기: 이 저장소의 `0.3` 라인은 v5(공개 런칭 준비) 작업에 해당합니다. v6부터는 `0.4`에서
> 이어집니다. 현재 상태는 **launch-ready(런칭 준비 완료)이며 실제 공개 게이트는 아직 켜지지
