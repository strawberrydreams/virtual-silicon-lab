...
에디터 읽기 전용 미리보기를 모바일에 대응(반응형)** 시켰습니다.

> 버전 표기: 이 저장소의 `0.4` 라인은 v6(모바일/반응형) 작업에 해당합니다(`0.3` 라인은 v5,
> 공개 런칭 준비). 현재 상태는 **launch-ready(런칭 준비 완료)이며 실제 공개 게이트는 아직 켜지지
> 않았습니다** — production 전환은 별도 운영 결정(아래 "런칭 상태" 참고)입니다.
