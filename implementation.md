...
## V8-M2 Prompt → Chip Layout (2026-06-20)

스펙 `docs/superpowers/specs/2026-06-20-v8-m2-prompt-chip-layout-design.md`, 계획
`docs/superpowers/plans/2026-06-20-v8-m2-prompt-chip-layout.md`를 기준으로 구현했다.

- **실행 기준선.** 기존 `v8-ai-assisted-creation` 브랜치에서 client 94 files/475 tests, server 68 files/264
  tests가 통과한 상태로 시작했다. 사용자 소유의 미커밋 로드맵 문서 3개는 범위 밖이므로 수정·스테이징하지 않는다.
- **draft theme 안전 경계.** `AiChipDraft`에 persisted schema가 아닌 optional `theme`을 추가하고,
  `mapAiDraftToProject`가 `StyleTheme` allowlist에 포함된 값만 적용하도록 했다. 누락·잘못된 값은 기존
  `createProject` 기본값 `neon`을 유지하므로 M0의 valid-project 보증과 `schemaVersion` 5는 바뀌지 않는다.
- **AI draft materialization.** 네트워크에서 받은 snapshot을 `migrateProject`로 재검증한 뒤 deep clone하고
  fresh id/`now` timestamp를 부여하는 순수 `materializeAiDraftProject`를 추가했다. 계획의 예시 구현은 입력에
  `remixedFrom`이 있을 때 그대로 보존할 수 있었지만, AI 생성은 원본 창작이라는 스펙을 강제하기 위해 clone에서
  lineage를 명시적으로 제거한다. AI가 고른 이름은 suffix 없이 보존한다.
- **로컬 저장 경로.** `projectStore.createFromAiDraft(snapshot)`이 materializer를 호출해 새 id의 프로젝트를
  repository에 저장하고 목록 맨 앞에 추가한다. 동일 server draft를 여러 번 전달해도 서로 다른 local id를
  사용하며, 저장 실패 전에는 store 목록을 변경하지 않는 기존 store command 순서를 유지한다.
- **provider theme 전달.** fake provider는 prompt에서 `mono`→`retro`→`military`→`keynote` 순으로 키워드를
  판별하고 나머지는 `neon`으로 결정해 로컬·테스트 환경에서도 재현 가능한 theme을 낸다. Anthropic adapter는
  기존 `output_config.format` JSON schema의 optional property로 같은 5개 theme enum을 추가하고 prompt에도
  선택지를 명시했다. `required`는 기존 `dieShape`/`blocks` 그대로여서 theme 누락은 mapper 기본값으로 안전하게
  처리된다. M0의 endpoint/quota/log와 DB migration은 변경하지 않았다.
- **클라이언트 API.** `liveAiDraftApi.generateDraft(prompt)`가 M0의 `POST /api/ai/generate-draft`를 호출하고
  `{ project }`를 반환한다. M1의 `AiApiError`/`AiServerUnreachableError` 클래스를 재사용해 401/429 같은 JSON
  error code는 보존하고, fetch 실패와 502/503/504는 서버 접근 불가로 통일했다. 클라이언트에는 provider SDK나
  API key가 추가되지 않는다.
- **dashboard prompt → editor.** dashboard action 영역에 prompt input과 `Generate with AI` 버튼을 추가했고,
  `App`에서 `generateDraft → createFromAiDraft`를 합성했다. 성공 시 fresh local project의 editor route로
  이동하며, 빈 prompt/처리 중에는 버튼을 비활성화한다. 실패는 401/429/503/unknown을 사용자가 행동할 수 있는
  inline `role="alert"` 문구로 바꾸고 navigation이나 local save를 수행하지 않는다. 기존 action row의 desktop
  정렬을 유지하면서 error가 header의 별도 flex item이 되지 않도록 actions wrapper를 추가했고, mobile에서는
  input/button을 full-width로 배치하는 최소 CSS를 함께 넣었다.
- **도구 편차.** 계획이 Task 4 전에 요구한 `claude-api` 스킬은 현재 환경의 설치 목록에 없다. Anthropic
  adapter 변경은 새 SDK 패턴을 도입하지 않고, 설치된 SDK 타입과 기존 `output_config.format` 구현 및 SDK mock
  테스트를 근거로 additive schema property만 추가한다.
- **자동 게이트/키 유출 점검.** 최종 `npm test`는 client 96 files/488 tests, server 68 files/267 tests;
  `npm run build`, `npm run typecheck --workspace server`, `npm run lint`가 모두 통과했다. build에는 기존의
  >500 kB chunk 경고만 남았고, `dist/assets`에서 `ANTHROPIC_API_KEY` 검색 결과는 0건이었다. 최종 lint에서
  lineage 제거용 미사용 destructuring binding과 API 테스트의 미사용 class import를 발견해, clone 후
  `delete migrated.remixedFrom` 표현과 필요한 import만 남기는 방식으로 수정했다.
- **브라우저 QA 보류.** 로그인 → dashboard prompt 입력 → Generate with AI → AI theme이 적용된 fresh local
  project가 editor에서 열리는 흐름, 그리고 서버 중단 시 inline error + project 미생성을 확인하는 실제 브라우저
  검증은 계획대로 **오너 수동 QA로 보류**한다. 이번 완료 근거는 fake-provider route/component/store 테스트와
  위 자동 게이트이며, 브라우저 세션을 수행했다고 주장하지 않는다.

## V8-M3 AI Layout Suggestions (2026-06-20)

스펙 `docs/superpowers/specs/2026-06-20-v8-m3-ai-layout-suggestions-design.md`, 계획
`docs/superpowers/plans/2026-06-20-v8-m3-ai-layout-suggestions.md`를 기준으로 구현했다.

- **실행 기준선.** 기존 `v8-ai-assisted-creation` 브랜치에서 client 96 files/488 tests, server 68 files/267
  tests가 통과한 상태로 시작했다. 사용자 소유의 미커밋 로드맵 문서 3개는 범위 밖이므로 수정·스테이징하지 않는다.
- **layout context 경계.** 순수 `deriveAiLayoutContext`가 현재 `Project`의 die shape과 block pixel rect를
  fractional rect로 변환한다. 서버에는 편집 가능한 전체 project가 아니라 제안에 필요한 최소 layout context만
  전달하며, persisted schema나 export 경로는 변경하지 않는다.
- **apply-time valid-output 경계.** 순수 `resolveAiSuggestionBlock`이 16개 block type allowlist 밖의 제안을
  `null`로 버리고, `blockClamp`가 fractional 좌표·크기를 die pixel bounds 안으로 clamp한다. oversized,
  out-of-bounds, non-finite 좌표와 24px 미만 크기까지 테스트로 고정했다. `blockClamp`의 allowlist/clamp는 현재
  M2 `mapAiDraftToProject`의 inline 로직과 중복되지만, M3 범위에서 기존 valid-project 경계를 재구성하지 않도록
  별도 모듈로 두고 후속 집중 cleanup에서 통합한다.
- **undo 가능한 적용 command.** `editorStore.applyAiSuggestion`이 resolver 결과만 기존 `commit()` 경로로
  추가하고 새 block을 선택한다. global-reflow 프로젝트에서는 제안 좌표를 target으로 기존 reflow를 적용하되,
  Accept 한 번은 정확히 history 한 건만 만들며 undo 한 번으로 추가 전 Project를 복원한다. unknown type은
  `null` no-op이어서 project/history 모두 바뀌지 않는다.
- **provider 확장.** `AiProvider.generateLayoutSuggestions`가 `{ context }`를 받아 `{ suggestions }`를 반환한다.
  fake provider는 현재 block type을 제외한 `Cache`/`GPU`/`PLL` 후보에서 최대 2개를 결정적으로 생성한다.
  Anthropic adapter는 기존 `output_config.format` JSON schema 패턴을 그대로 확장해 suggestion array를 받고,
  refusal/no-text는 기존 메서드와 동일하게 throw한다. 계획이 요구한 `claude-api` 스킬은 설치되어 있지 않아
  설치된 SDK 타입, 기존 두 adapter 메서드, SDK mock 테스트를 근거로 구현했다.
- **계획 보정.** provider interface에 필수 메서드를 추가하자 기존 generate-copy route 테스트의 수동 failing
  provider fixture가 새 메서드를 구현하지 않아 server typecheck가 실패했다. 계획은 Task 5 완료 시 typecheck
  green을 예상했지만 이 fixture 갱신을 누락했으므로, production 변경 없이 `generateLayoutSuggestions` throw
  stub을 추가해 interface contract를 맞췄다.
- **build 호환 보정.** 계획의 editor store 테스트 예시가 `Block[].at(-1)`을 사용했지만 현재 client TypeScript
  lib target에는 ES2022 `Array.prototype.at`이 없어 최종 `tsc -b`가 실패했다. runtime 의미가 같은
  `added[added.length - 1]`로 바꿔 target/lib 확장 없이 기존 브라우저 호환 계약을 유지했다.
- **`POST /api/ai/suggest-layout`.** M0/M1의 인증·`requireUserWithinQuota`·24시간 shared quota를 그대로
  재사용하고, provider 호출 전에 `ai_prompt_log.kind='suggest-layout'`로 context를 기록한다. 성공은
  `{ suggestions }`, 미인증/누락 context/quota/provider throw는 각각 401/400/429/503이며 서버에는 project나
  suggestion을 저장하지 않는다. 새 endpoint 외 DB migration과 quota 정책 변경은 없다.
- **클라이언트 API.** `liveAiSuggestApi.generateSuggestions(context)`가 suggest-layout endpoint를 호출하고
  suggestion array만 반환한다. M1의 `AiApiError`/`AiServerUnreachableError`를 재사용해 JSON error code는
  보존하고, fetch 실패와 502/503/504는 접근 불가로 통일한다. provider SDK나 key는 client에 import하지 않는다.
- **inspector 제안 UI.** lazy `AiLayoutSuggestionsPanel`을 Advanced Controls에 M1 `AiSpecPanel`과 나란히
  추가했다. 버튼을 눌러야 context를 파생하고 요청하며, 각 항목의 Accept는 `applyAiSuggestion`을 한 번 호출한
  뒤 목록에서 제거하고 Reject는 callback 없이 제거한다. 401/429/503/offline은 panel 내부 `role="alert"`로만
  표시되어 canvas·palette·수동 inspector 편집을 막지 않는다. React 상태는 interaction handler에서만 갱신하고
  suggestion 제거는 functional setState로 처리했다.
- **UI 검증 범위.** 승인 계획이 browser QA를 owner-manual/do-not-automate로 고정했으므로 이번 구현에서는 RTL로
  fetch→list→Accept/Reject와 quota/offline inline error, rail mount를 검증하고 실제 브라우저 세션은 수행하지 않는다.
- **자동 게이트/키 유출 점검.** 최종 `npm test`는 client 100 files/504 tests, server 68 files/275 tests이며,
...
- **브라우저 QA 보류.** editor chip 열기 → Suggest improvements → Accept로 block 추가 → undo로 제거,
  Reject 시 project 불변, 서버 중단 시 inline error와 수동 편집 유지 흐름은 승인 계획대로 **오너 수동 QA로
  보류**한다. 이번 완료 근거는 전체 자동 게이트와 fake-provider route/API/store/RTL 테스트이며, 실제 브라우저
  세션을 수행했다고 주장하지 않는다.

## V8-M4 AI Remix / Variations (2026-06-20)

스펙 `docs/superpowers/specs/2026-06-20-v8-m4-ai-variations-design.md`, 계획
`docs/superpowers/plans/2026-06-20-v8-m4-ai-variations.md`를 기준으로 구현했다.

- **variation context 경계.** 순수 `deriveAiVariationContext`가 M3의 fractional full-layout context에 source
  project의 `name`과 `theme`을 더한다. 원본은 입력으로만 읽고, 새 in-memory context 외 persisted schema나
  export 계약은 변경하지 않는다.
- **provider와 valid-project 경계.** `AiProvider.generateVariations`는 `{ context, count }`에서
  `{ variations: AiChipDraft[] }`를 반환한다. fake provider는 원본 layout을 유지하면서 5개 theme을 결정적으로
  순환하고, Anthropic adapter는 기존 `output_config.format` JSON schema 패턴으로 variation array를 받는다.
  각 draft는 route에서 기존 `mapAiDraftToProject`를 통과하므로 unknown block 제거, 좌표 clamp, 순차 z-order,
  valid-or-default theme, `schemaVersion` 5 보증을 그대로 재사용한다. 계획이 요구한 `claude-api` 스킬은 설치되어
  있지 않아 기존 adapter 세 메서드와 설치된 SDK 타입, SDK mock 테스트를 근거로 확장했다.
- **`POST /api/ai/generate-variations`.** M0의 인증·`requireUserWithinQuota`·shared 24시간 quota를 재사용하며,
  provider 호출 전에 `ai_prompt_log.kind='generate-variations'` 한 행을 기록한다. N과 무관하게 호출당 quota 한
  단위다. count는 2–4로 제한되고 누락·non-finite·0·음수는 기본값 3으로 처리한다. 계획 샘플은 `0`을 2로 clamp해
  설계의 "invalid → default 3"과 충돌했으므로 설계를 우선해 route 테스트로 보정했다. 신규 migration이나 서버
  project 저장은 없다.
- **클라이언트와 독립 로컬 저장.** `liveAiVariationsApi`는 기존 `AiApiError`와
  `AiServerUnreachableError`를 동일 class로 재사용한다. `AiVariationsPanel`은 2–4 count 선택, lazy generate,
  variation별 name/theme/block count와 mini preview, 개별 `Save as new project`를 제공한다. Save는 기존
  `projectStore.createFromAiDraft`를 호출해 migrate/deep-clone/fresh id로 독립 local project를 만들고 navigation
  없이 `Saved ✓`로 바뀐다. 원본 editor project는 불변이며 실패는 inline alert로만 표시된다.
- **thumbnail 재사용 trade-off.** 스펙의 신규 `ChipVariationThumbnail`/fit helper 대신 계획 검토에서 이미
  검증된 read-only Konva `MobileChipPreview`와 shared `ChipArtwork`를 재사용했다. 중복 fit math와 컴포넌트를
  만들지 않으며, jsdom RTL에서는 canvas 제약 때문에 preview만 mock한다.
- **계획 보정.** `EditorPage`에 필수 `onSaveVariation` prop을 추가한 뒤 계획에 없던 기존
  `EditorPage.test.tsx` render fixture 세 곳이 build typecheck에서 실패했다. production prop을 optional로
  약화하지 않고 fixture에 `vi.fn()`을 전달해 실제 계약과 맞췄다.
- **자동 게이트/키 유출 점검.** 최종 `npm test`는 client 103 files/513 tests, server 68 files/285 tests이며,
  `npm run build`(기존 >500 kB 청크 경고만), `npm run typecheck --workspace server`, `npm run lint`가 모두
  통과했다. 빌드 산출물 `dist/assets`에서 `ANTHROPIC_API_KEY` 문자열은 0건이었다.
- **브라우저 QA 보류.** editor에서 count 2–4 선택 → Generate variations → N개 thumbnail 확인 → 한 variation
  Save → dashboard에 독립 project 생성 및 source 불변 확인, 서버 중단 시 inline 접근 불가 문구와 수동 편집
  유지를 확인하는 실제 브라우저 검증은 승인 계획대로 **오너 수동 QA로 보류**한다. 완료 근거는 전체 자동
  게이트와 fake-provider route/API/panel 테스트이며, 실제 브라우저 세션을 수행했다고 주장하지 않는다.

## V8-M5 QA & Cost Hardening (2026-06-20)

스펙 `docs/superpowers/specs/2026-06-20-v8-m5-qa-cost-hardening-design.md`, 계획
`docs/superpowers/plans/2026-06-20-v8-m5-qa-cost-hardening.md`를 기준으로 구현했다. 이 단계로 v8의 계획된
M0–M5를 닫는다.

- **abuse 입력 경계.** `server/src/ai/routes.ts`의 `MAX_PROMPT_LENGTH=2000`과
  `MAX_CONTEXT_BLOCKS=64`가 generate-draft/copy, suggest-layout, generate-variations 네 route의 입력을
  제한한다. 인증·기존 quota 확인 후 payload를 파싱하되 `logPrompt`/provider 호출 전 oversized 입력을
  `400 PAYLOAD_TOO_LARGE`로 거절하므로 quota를 소비하거나 로그를 오염시키지 않는다. Dashboard prompt input에도
  `maxLength={2000}`을 추가했지만 서버 검증이 최종 권위다.
- **burst rate limit.** production `SENSITIVE_RATE_LIMIT_OVERRIDES`에 네 AI POST path를 각각
  `{ windowMs: 60_000, max: 10 }`으로 추가했다. 기존 IP별 app middleware를 재사용하는 burst cap이며,
  사용자별 shared 24시간 quota(기본 20)는 그대로다. dev/test의 rate-limit 비활성 계약도 바꾸지 않았다.
- **비용/사용량 read.** 신규 `summarizeAiUsage`가 기존 `ai_prompt_log`를 지정한 inclusive window에서 읽어
  `totalCalls`, `distinctUsers`, `byKind`를 집계한다. `GET /api/ai/usage?windowHours=H`는 signed session과
  `isAdminEmail`을 inline으로 검사해 anonymous 401/non-admin 403/admin 200을 반환하고, H는 기본 24시간·최대
  720시간이다. 신규 migration/column이나 admin UI는 없으며 JSON read endpoint만 추가했다.
- **전체 AI surface 회귀.** fake-provider E2E가 signup 후 generate-draft/copy, suggest-layout,
  generate-variations를 모두 호출해 valid output과 prompt log 4행을 검증하고, 서로 다른 kind 네 건 뒤 다섯 번째
  호출이 shared quota 429가 되는 것을 고정한다. 기존 route 테스트는 네 oversized 입력 모두 400이고 log 0건임을
  검증한다.
- **범위와 버전.** README version line을 `0.6 v8`로 올렸고 `package.json`은 1.0.0을 유지했다. DB migration,
  `Project` schemaVersion 5, `src/domain/`, Konva export, local-first 저장 계약, Anthropic adapter는 변경하지 않았다.
