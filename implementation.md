...
- Added the `'freeform'` case to `resolveDieOutline` in `src/domain/die/dieOutline.ts`: normalized vertices are scaled by `die.width`/`die.height` into absolute points, the centroid is the plain mean of those points (not the bounding-box center, matching how other polygon shapes compute centroid), and the result is returned through the existing `polygonOutline(points, centroid)` helper used by every other polygon-based shape.
...
- Downstream RUNTIME polygon pipeline is unchanged by design: freeform resolves to a plain `Point[]` polygon exactly like every other non-rect shape, so `src/features/editor/canvas/ChipArtwork.tsx` and `src/three/chip3dModel.ts` needed zero changes (confirmed via `git diff` against `main`), and `clampBlockToPolygon`/extrusion continue to consume whatever `resolveDieOutline` returns without knowing a shape is freeform.
- Final verification:
