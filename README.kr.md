...
저장/초기화를 폰 너비 에디터에 제공하되 모바일 2D canvas 작도는 열지 않습니다.

> 버전 표기: 이 저장소의 `0.9` 라인은 v11(Mobile 3D Authoring)에 해당합니다(`0.8` 라인은 v10
> 3D Authoring, `0.7` 라인은 v9 Deep Canvas, `0.6` 라인은 v8 AI 보조 생성, `0.5` 라인은 v7
> 비주얼 뎁스, `0.4` 라인은 v6 모바일/반응형, `0.3` 라인은 v5 공개 런칭 준비). 2D Konva
> 작도·PNG 내보내기 계약은 그대로이며, v11 모바일 작도는 publish snapshot에 함께 실리는 동일한
> additive 클라이언트 `scene3d` 프로젝트 JSON을 씁니다. 실제 공개 게이트는 아직 켜지지 않았습니다 —
...
v9(Deep Canvas), v10(3D Authoring), v11(Mobile 3D Authoring)은 런칭 게이트를 바꾸지 않습니다 —
AI는 명시적 provider/key 뒤의 서버 전용이고, v9/v10/v11은 서버 route·SQLite migration 없는
클라이언트 작도입니다. 실제 production
전환(`VSL_ACCESS_MODE=invite`)은 배포 환경 구축 + 소유자 go/no-go 이후의 운영 동작으로 남겨져
있습니다. 운영 문서는 `docs/ops/`(런북·백업/복원·QA 체크리스트·v11 mobile 3D authoring release QA)에
있습니다.
