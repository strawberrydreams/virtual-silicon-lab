...
에디터 전용 앰비언트 모션으로 2D 작도 표면을 사각형 너머로 확장했으며**, **v10 3D Authoring**은
카메라·조명·환경·애니메이션·전체 scene look preset을 저장하는 3D presentation 작도 표면으로
쇼케이스를 확장했고, **v11 Mobile 3D Authoring**은 모바일 look preset·조명 chip·터치 카메라
저장/초기화를 폰 너비 에디터에 제공하되 모바일 2D canvas 작도는 열지 않습니다. 이제
**v12 Continuum Sync**는 로그인한 사용자의 로컬 프로젝트가 디바이스 사이를 따라가도록 opt-in
멀티 디바이스 동기화를 추가하며, 익명 사용은 완전히 로컬로 유지됩니다.

> 버전 표기: 이 저장소의 `0.10` 라인은 v12(Continuum Sync)에 해당합니다(`0.9`는 v11, `0.8`은
> v10, `0.7` 라인은 v9 Deep Canvas, `0.6` 라인은 v8 AI 보조 생성, `0.5` 라인은 v7 비주얼 뎁스,
> `0.4` 라인은 v6 모바일/반응형, `0.3` 라인은 v5 공개 런칭 준비). 2D Konva 작도·PNG 내보내기
> 계약은 그대로이며, v12 sync는 로그인 사용자에게만 로컬 프로젝트 JSON을 미러링합니다. 실제 공개
> 게이트는 아직 켜지지 않았습니다 — production 전환은 별도 운영 결정(아래 "런칭 상태" 참고)입니다.
...
- **v10 3D Authoring** — 보기 전용 3D 쇼케이스를 에디터 작도 표면으로 확장했습니다. 작성자는
  정규화된 hero camera, 조명 preset + intensity, 환경/post override, 결정론적 turntable/glow
  애니메이션, 그리고 한 번에 적용되는 전체 scene look preset을 저장할 수 있습니다. 동일한 순수
  `resolveScene3D` descriptor가 에디터·갤러리·공유 deep link·MP4 capture를 구동하고, 2D die/poster
  PNG 계약은 그대로입니다. 최종 QA: `docs/ops/v10-3d-authoring-qa.md`.
- **v11 Mobile 3D Authoring** — 모바일 에디터 route가 동일한 로컬 editor store와 저장 경로를
  사용해 curated 3D subset을 제공합니다: 모바일 look preset·조명 chip·터치 orbit camera 저장/초기화,
  compact accessible control rail, 기존 unavailable fallback. 2D Konva 작도는 desktop-only로 유지하며
  backend route, SQLite migration, schema 변경은 없습니다. 최종 QA:
  `docs/ops/v11-mobile-3d-authoring-qa.md`.
- **v12 Continuum Sync** — 로그인한 사용자는 새 사용자별 `synced_projects` table과 `/api/sync/*`
  route를 통해 로컬 프로젝트를 last-write-wins 멀티 디바이스 동기화로 미러링합니다. 익명 사용자는
  그대로 순수 로컬이며, header badge가 sync 상태를 표시합니다. 최종 QA:
...
- v11은 모바일 look preset·조명 chip·터치 카메라 저장/초기화를 제공합니다.
...
→ 밴 → 프로필/SEO → 재설정 플로우는 단위·통합 테스트와 브라우저 QA로 검증되었습니다. v8(AI),
v9(Deep Canvas), v10(3D Authoring), v11(Mobile 3D Authoring)은 런칭 게이트를 바꾸지 않았습니다 —
AI는 명시적 provider/key 뒤의 서버 전용이고, v9/v10/v11은 서버 route·SQLite migration 없는
클라이언트 작도였습니다. v12는 인증된 사용자별 서버 sync 표면(`synced_projects` + `/api/sync/*`)을
추가해 로그인한 프로젝트가 계정 기반으로 디바이스 사이를 따라가게 하지만, 여전히 invite-paced,
SQLite-backed, best-effort이며 익명 사용은 완전히 로컬입니다. 실제 production
전환(`VSL_ACCESS_MODE=invite`)은 배포 환경 구축 + 소유자 go/no-go 이후의 운영 동작으로 남겨져
있습니다. 운영 문서는 `docs/ops/`(런북·백업/복원·QA 체크리스트·v12 continuum sync release QA)에
있습니다.
